...
def get_joint_rssi_new(trip, exclude_unmatched = True, include_pretrip = True, only_dominant_major = True, 
                   normalize_zero = True, zero_val = -100, exclude_zeros = False):
    data = prelim_filter(trip, include_pretrip, only_dominant_major, normalize_zero, zero_val)
    ...
    m1 = m1.groupby(['timestamp', 'minor'], as_index = False).agg({
        'username': 'first',
        'latitude': 'mean',
        'longitude': 'mean', 
        'speed': 'mean',
        ...
        'gravity_accel_x': 'mean',
        'gravity_accel_y': 'mean', 
        'gravity_accel_z': 'mean', 
        'user_accel_x': 'mean', 
        'user_accel_y': 'mean',
        'user_accel_z': 'mean', 
        'magnetic_field_x': 'mean', 
        'magnetic_field_y': 'mean',
        'magnetic_field_z': 'mean'
    })
    m2 = m2.groupby(['timestamp', 'minor'], as_index = False).agg({
        'username': 'first',
        'latitude': 'mean',
        'longitude': 'mean', 
        'speed': 'mean',
...
def get_tagged_dataset(trips, n = 1, exclude_unmatched = True, include_pretrip = False, only_dominant_major = True, 
    ...
    for i, trip in enumerate(trips):
        if trip.seat != "none" and trip.data.shape[0] > 0:
            ...
            join["rssi_diff"] = join["rssi_1"] - join["rssi_2"] #Trying rssi_2 - rssi_1
    ...
    if aggregate_feats:
        for i in range(len(df)):
            ...
            # Calculating aggregate metrics for measurements with x/y/z components
            frame["user_accel_magnitude"] = np.sqrt(frame["user_accel_x"]**2 + frame["user_accel_y"]**2 + frame["user_accel_z"]**2)
            frame["rotation_rate_magnitude"] = np.sqrt(frame["rotation_rate_x"]**2 + frame["rotation_rate_y"]**2 + frame["rotation_rate_z"]**2)
...
def get_tagged_dataset_new(trips, n = 1, exclude_unmatched = True, include_pretrip = False, only_dominant_major = True, 
                   normalize_zero = True, zero_val = -100, aggregate_feats = True, exclude_null_trips = True, exclude_zeros = False, trim_end_zeros = False, trim_all_zeros = False, trim_double_zeros = False,
                   normalize_acc = False, acc_val = 1):
    df = []
    seat_to_num = {"front" : 0, "middle" : 1, "back" : 2}
    ...
    for i, trip in enumerate(trips):
        if trip.seat != "none" and trip.data.shape[0] > 0:
            join = get_joint_rssi_new(trip, exclude_unmatched, include_pretrip, only_dominant_major, normalize_zero, zero_val, exclude_zeros)
            join["seat"] = seat_to_num[trip.seat]
            join["rssi_diff"] = join["rssi_1"] - join["rssi_2"] #Trying rssi_2 - rssi_1
            if normalize_acc and join.shape[0] > 0:
                if acc_val == 1:
    ...
    for user in new:
        ...
        if len(user) == 0:
            ...
            for trip in user:
                ...
                data.loc[:, "rssi_diff"] = data.loc[:, "rssi_1"] - data.loc[:, "rssi_2"]  #Trying rssi_2 - rssi_1
...
def get_average_dataset(trips, n, exclude_unmatched = True, include_pretrip = True, only_dominant_major = True, 
    ...
    for trip in trips:
        if trip.seat != "none" and trip.data.shape[0] > 0:
            join = get_joint_rssi(trip, exclude_unmatched, include_pretrip, only_dominant_major, normalize_zero, zero_val, exclude_zeros)
            join["seat"] = trip.seat
            join.loc[:, "rssi_diff"] = join.loc[:, "rssi_1"] - join.loc[:, "rssi_2"] #Trying rssi_2 - rssi_1
            join["trip_idx"] = trip.trip_idx 
            df.append(join)
