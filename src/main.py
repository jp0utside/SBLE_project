...
def main():
    ...
    trips = get_trips_from_files(sble, notif)

    data = get_tagged_dataset(trips)

    save_dfs_to_csvs(data, suffix = '_rotation')
...
def comparison():
    ...
    old_data = get_tagged_dataset(trips)
    new_data = get_tagged_dataset_new(trips)

    X = [frame[all_features] for frame in old_data]
    y = [frame.iloc[0]["seat"] if frame.shape[0] > 0 else -1 for frame in old_data]

    cv = StratifiedKFold(n_splits = 5, random_state = 33, shuffle = True)

    lstm_old = SklearnLSTMWrapper(features = rssi_features, scaler = StandardScaler(), pca = PCA(n_components = 5), hidden_size = 50, lr = 0.001, num_epochs = 5, sub_sequence_length = 12, batch_size = 25, num_layers = 1, bidirectional = False, recurrent_dropout = 0.25, l2_lambda = 0.05)
    old_results = analyze_lstm(lstm_old, X, y, cv)

    X = [frame[all_features] for frame in new_data]
    y = [frame.iloc[0]["seat"] if frame.shape[0] > 0 else -1 for frame in new_data]

    lstm_new = SklearnLSTMWrapper(features = rssi_features, scaler = StandardScaler(), pca = PCA(n_components = 5), hidden_size = 50, lr = 0.001, num_epochs = 5, sub_sequence_length = 12, batch_size = 25, num_layers = 1, bidirectional = False, recurrent_dropout = 0.25, l2_lambda = 0.05)
    new_results = analyze_lstm(lstm_new, X, y, cv)
