...
def get_trips_from_files(sble_data = None, notif_data = None):
    ...
    for t in trips:
        ...
        if (t.data.shape[0] > 0) and (t.notifs.shape[0] > 0):
            ...
            t.end = last_time
            if len(t.data["major"].unique()) == 1:
                t.major = t.data["major"].unique()[0]
    
    add_trip_numbers(trips)
    return trips


def save_dfs_to_csvs(data, dir_path = "/Users/Jake/Computer Science/SBLE_project/cleaned_trips/", 
                   suffix = '',
                   columns = ['Timestamp', 'Username', 'Major', 'Seat', 
                              'Latitude', 'Longitude', 'Rssi_diff',
                              'Rssi_1', 'Rssi_accuracy_1',
                              'Rssi_2', 'Rssi_accuracy_2', 'Trip_idx',
                              'Speed', 'User_accel_magnitude', 'Rotation_rate_magnitude']):
    for i,frame in enumerate(data):
        frame = clean_columns(frame)
        frame = frame[columns]

        idx = frame["Trip_idx"].unique()[0]
        user = frame["Username"].unique()[0]
        major = frame["Major"].unique()[0]
        seat = frame["Seat"].unique()[0]


        frame.to_csv(dir_path + "t{}_{}_bus{}_s{}{}.csv".format(idx, user, major, seat, suffix), index = False)
        

def clean_columns(frame, caps = True):
    cols = list(frame.columns)
    if 'username_1' in cols and 'username_2' in cols:
        frame = frame.drop('username_2', axis=1)
    
    if 'major_1' in cols and 'major_2' in cols:
        frame = frame.drop('major_2', axis=1)
    
    if 'minor_1' in cols:
        frame = frame.drop('minor_1', axis=1)
    
    if 'minor_2' in cols:
        frame = frame.drop('minor_2', axis=1)

    frame = frame.rename(columns={'username_1': 'username', 'username_2': 'username',
                                'major_1': 'major', 'major_2': 'major'})
