...
Plotted lines can be chosen to reflect either pre or post trip sections or RSSI differences.

Args:
    trips: array of trip objects to be plotted
    fig: figure object to apply the plot to
    ax: axes object to apply the plot to
...
"""
def graph_trips_on_map(trips, color_coding = 0, fig_height = 10):
    gis = GIS(api_key = "AAPTxy8BH1VEsoebNVZXo8HurGcMZQkF_uo1HnCVSv2M-2KoaGxDI4zssBgsEUhwzp51BfgP7rHyCUeBOV0gVfUZHFRhducjt0vWRJ3AjSHv7CNt069jXF_JQskIX5_k9s3UFk6pId3PKhZ7fIMj1kcAebx08x7VUeMu00QVyOvv3_XbxsEo0lllrDosz0-RLC_zqRuBeFdsfRDdvNoVOOLE0Yb519VucIW1Wt4jy7ENmxs.AT1_SxdlZelX")

    map = gis.map()
    map.basemap.basemap = "hybrid"

    center_lat = 36.990000
    center_lon = -122.058400

    lat_buffer = 0.014
    lon_buffer = 0.0105
    
    # upper_left = [37.002708, -122.072946]
    # bottom_right = [36.977255, -122.046818]
    xmin = center_lon - lon_buffer
    xmax = center_lon + lon_buffer
    ymin = center_lat - lat_buffer
    ymax = center_lat + lat_buffer

    extent = {'spatialReference':{'latestWkid': 3857, 'wkid': 102100},
              'xmin': lon_to_merc(xmin), 
              'ymin': lat_to_merc(ymin), 
              'xmax': lon_to_merc(xmax), 
              'ymax': lat_to_merc(ymax)}

    width = 900
    height = 1200
    
    url = map.print(file_format="PNG32", extent=extent, output_dimensions=(width, height))
    ...
    for i,trip in enumerate(trips):
        fig, ax = plt.subplots(figsize = (fig_height, fig_height*aspect_ratio), dpi = 100)
        print(i)
        ax.imshow(img, extent = [xmin, xmax, ymin, ymax], aspect = 'equal')
        ax.set_xlim(xmin, xmax)
        ax.set_ylim(ymin, ymax)

        if trip.data.shape[0] > 0:
            print("     pass")
            # data["longitude"] = data["longitude"].apply(lambda x: ((lon_to_merc(x) - xmin)/xrange)*width)
            # data["latitude"] = data["latitude"].apply(lambda x: ((lat_to_merc(x) - ymin)/yrange)*height)
            ...
            if color_coding == 0:
                ...
                if pre_trip.shape[0] > 0:
                    ax.scatter(pre_trip["longitude"], pre_trip["latitude"], color="red", s = 10, label = "Pre-Trip")
                if mid_trip.shape[0] > 0:
                    ax.scatter(mid_trip["longitude"], mid_trip["latitude"], color="blue", s = 10, label = "Mid-Trip")
                
                plt.show()
            elif color_coding == 1:
                data = get_joint_rssi(trip, exclude_unmatched=True, include_pretrip=True, only_dominant_major=True, normalize_zero=True, zero_val=-100, exclude_zeros=False)
                data["rssi_diff"] = data["rssi_1"] - data["rssi_2"]
                data["longitude"] = (data["longitude_1"] + data["longitude_2"])/2
                data["latitude"] = (data["latitude_1"] + data["latitude_2"])/2
                ...
                plt.title("Trip: {}, User: {}, Seat: {}".format(trip.trip_idx, trip.user, trip.seat))
                cbar = plt.colorbar(sc)
                cbar.set_label('RSSI Diff (Front - Back)')
                plt.show()
...
"""
Function to convert latitude coordinates to corresponding mercator projection coordinates.

Args:
    lat: float representing latitude measurement.

Returns:
    y: float representing cooresponding mercator projection measurement.
"""
def lat_to_merc(lat):
...
"""
Function to convert longitude coordinates to corresponding mercator projection coordinates.

Args:
    lon: float representing longitude measurement.
